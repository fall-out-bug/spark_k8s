...
CONFIG = {
    ...
    "mape_threshold": 0.30,  # 30% MAPE threshold for limited demo data
}
...
def build_spark_submit_pod_task(task_id: str, script_name: str, extra_env: dict | None = None):
    ...
    return KubernetesPodOperator(
        ...
        cmds=["/bin/bash", "-lc"],
        arguments=[download_and_run_cmd],
        env_vars=env_vars,
...
def train_borough_model(borough: str, **context):
    ...
    if len(df_borough) < 100:
        ...
        push_metric(
            "ml_training_samples", len(df_borough), {"borough": borough.lower().replace(" ", "_"), "stage": "training"}
        )
    ...
    # Train revenue model
    model_revenue = GradientBoostingRegressor(n_estimators=100, max_depth=5, learning_rate=0.1, random_state=42)
    model_revenue.fit(X_train, y_rev_train)

    # Train trips model
    model_trips = GradientBoostingRegressor(n_estimators=100, max_depth=5, learning_rate=0.1, random_state=42)
...
def validate_models(**context):
    ...
    for borough in boroughs:
        try:
            ...
            try:
                response = s3.get_object(Bucket="ml-models", Key=metrics_key)
                model_data = json.loads(response["Body"].read().decode("utf-8"))
            except Exception:
                response = s3.get_object(
                    Bucket="ml-models", Key=f"taxi-predictor/v{CONFIG['model_version']}/{borough}.pkl"
                )
                model_data = pickle.loads(response["Body"].read())

            mape_rev = float(model_data.get("mape_revenue", 999.0))
            mape_trips = float(model_data.get("mape_trips", 999.0))

            passed = mape_rev < CONFIG["mape_threshold"] and mape_trips < CONFIG["mape_threshold"]
            if passed:
                passed_count += 1

            results.append({"borough": borough, "mape_revenue": mape_rev, "mape_trips": mape_trips, "passed": passed})
...
def generate_predictions(**context):
    ...
    for borough in boroughs:
        try:
            response = s3.get_object(
                Bucket="ml-models", Key=f"taxi-predictor/{CONFIG['model_version']}/{borough}.joblib"
            )
    ...
    s3.put_object(Bucket="nyc-taxi", Key=f"predictions/{CONFIG['model_version']}/7day_forecast.csv", Body=csv_buffer)
...
with DAG(
    ...
    # Task 2: Feature engineering on Spark Standalone (via K8s pod)
    feature_prep = build_spark_submit_pod_task(
        task_id="feature_engineering",
        script_name="taxi_feature_engineering.py",
    )

    # Task 3: Training on Spark Standalone (via K8s pod)
    train_models = build_spark_submit_pod_task(
        task_id="train_models",
        script_name="taxi_catboost_training.py",
    )
    ...
    # Task 5: Generate predictions on Spark Standalone (via K8s pod)
