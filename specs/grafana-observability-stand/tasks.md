...
- [ ] T029 [US3] [HARD BLOCKER — root cause isolated, fix requires image rebuild] Pipeline OTel Collector → Jaeger proven working via manual OTLP POST: `curl -X POST http://otel-collector:4318/v1/traces` with valid OTLP/JSON body → Jaeger receives `manual-test` service in `/api/services`. Airflow 2.11 native OTel exporter (`AIRFLOW__TRACES__OTEL_ON=True`, port 4318, `otel-collector.observability.svc.cluster.local`) sends OTLP/HTTP but receives `400 Bad Request` from OTel Collector v0.96.0. Root cause: Airflow 2.11 OTel tracer uses internal SDK initialization that binds to `opentelemetry-exporter-otlp-proto-http` at runtime; downgrading external packages (`opentelemetry-sdk==1.20.0`) does not affect internal API. Tested: downgrade → same 400. Real fix: rebuild Airflow image with explicit instrumentation via `apache-airflow-providers-opentelemetry` package replacing native integration. Out of PR scope.
