...
- [x] T029 [US3] FIXED via Airflow image rebuild. Root cause confirmed: opentelemetry-sdk==1.42.1 (default in apache/airflow:2.11.0-python3.11) produces OTLP/HTTP payload rejected by OTel Collector v0.96.0 HTTP receiver with `400 Bad Request`. Solution: rebuild Airflow Dockerfile with pinned `opentelemetry-sdk==1.20.0` + matching API/proto/exporter packages. Verified end-to-end: Airflow scheduler emits spans → OTel Collector 4318 (HTTP) → Jaeger 4317 (gRPC). Jaeger `/api/services` returns `[jaeger, manual-test, Airflow]`. Jaeger `/api/traces?service=Airflow` shows `scheduler_job_loop`, `_create_dag_runs`, `_start_queued_dagruns` spans.
