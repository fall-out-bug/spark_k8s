...
- [ ] T029 [US3] [PARTIAL — root cause isolated] Pipeline OTel Collector → Jaeger proven working via manual OTLP POST: `curl -X POST http://otel-collector:4318/v1/traces` with valid OTLP/JSON body → Jaeger receives `manual-test` service. Airflow 2.11 native OTel exporter (`AIRFLOW__TRACES__OTEL_ON=True`, port 4318) sends OTLP/HTTP but receives `400 Bad Request` from OTel Collector. Airflow 2.11 has no `otel_protocol` config (always HTTP path `/v1/traces`); `opentelemetry-exporter-otlp-proto-http==1.42.1` installed. Root cause: Airflow OTel exporter payload encoding mismatch with OTel Collector v0.96.0 HTTP receiver — manual JSON test succeeds, Airflow protobuf-encoded batch rejected. Follow-up options: (a) downgrade opentelemetry-sdk to 1.20.x in Airflow image, (b) replace Airflow OTel with explicit instrumentation via `apache-airflow-providers-opentelemetry` package, (c) patch OTel Collector HTTP receiver with lenient protobuf decoder.
