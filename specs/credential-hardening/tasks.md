...
- [ ] T001 [US1] Audit: `grep -rnE '$S3_ACCESS_KEY|hive123' charts/*/values*.yaml charts/*/presets/*.yaml > /tmp/cred-audit.txt` — capture baseline
- [ ] T002 [US1] Edit `charts/spark-3.5/values.yaml`: replace `$S3_ACCESS_KEY` → `""`, `hive123` → `""` + add comment "REQUIRED: set via --set or ExternalSecrets"
...
- [ ] T011 [US1] Add grep assertion test in `tests/security/` (or extend existing) — fail if `$S3_ACCESS_KEY`/`hive123` present in any `values*.yaml`
...
- [ ] T022 `grep -rE '$S3_ACCESS_KEY|hive123' charts/*/values*.yaml charts/*/presets/*.yaml` returns nothing
