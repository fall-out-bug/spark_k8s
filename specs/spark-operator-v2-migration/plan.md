...
> **Recovered 2026-08-09** from the abandoned `docs/spark-operator-plan` branch
> (commit 74eaa12, originally 2026-06-22). Aligns with the Q0 decision (kubeflow).
>
> **Staleness note:** since this plan was written, the standalone
> `charts/spark-operator/` already migrated its image to
> `ghcr.io/kubeflow/spark-operator/controller:v2.5.1` — so the "dead gcr.io image
> path" below now applies only to the per-version refs in
> `charts/spark-{3.5,4.0,4.1}/values.yaml` (still `ghcr.io/googlecloudplatform/...`).
> The core plan — replace the vendored chart with the upstream Helm dependency +
> remap values — remains the recommended implementation path.
