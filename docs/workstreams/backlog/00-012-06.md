...
- `docker/jupyter/Dockerfile` — Jupyter image with library versions
...
2. **Implement library compatibility test scenarios:**
   - `test_pandas_2x.py` — pandas 2.0.x compatibility
   - `test_pandas_1x.py` — pandas 1.5.x compatibility
   - `test_numpy_2x.py` — numpy 2.0.x compatibility
   - `test_numpy_1x.py` — numpy 1.24.x compatibility
   - `test_pyarrow_15x.py` — pyarrow 15.x compatibility
   - `test_pyarrow_14x.py` — pyarrow 14.x compatibility
...
- All 8 scenarios (6 modules × various version combos) pass
