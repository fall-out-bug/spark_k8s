...
**Acceptance Criteria:**
- [ ] AC1: Prometheus Helm chart создан
- [ ] AC2: JMX exporter pod sidecar настроен
- [ ] AC3: Spark metrics доступны в Prometheus
- [ ] AC4: K8s metrics (kube-state-metrics) работают
- [ ] AC5: Scrape интервал 15s
...
| # | Severity | Issue | Fix |
|---|----------|-------|-----|
| 1 | HIGH | helm dependency build not run | helm dependency build charts/observability/prometheus |
| 2 | HIGH | servicemonitor template uses spark.name (wrong scope) | Use observability chart values |
