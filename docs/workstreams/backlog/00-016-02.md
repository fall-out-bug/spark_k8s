...
- Loki Helm chart для лог aggregation
- Fluent Bit/Promtail настроен
- Structured JSON logs с trace ID
- Log sampling (INFO 10%, ERROR/WARN 100%)

**Acceptance Criteria:**
- [x] AC1: Loki Helm chart создан
- [x] AC2: Promtail collects logs from all pods
- [x] AC3: JSON structured logging работает
- [x] AC4: Trace ID correlation работает
- [x] AC5: Log sampling настроен
- [x] AC6: Grafana datasource настроен
...
- [x] AC1: Loki chart — ✅ Exists
- [x] AC2: Promtail template — ✅ promtail-spark.yaml
- [ ] AC3–AC6: JSON logging, trace ID, sampling — ⚠️ Template uses spark.name (wrong scope)
...
| # | Severity | Issue | Fix |
|---|----------|-------|-----|
| 1 | HIGH | promtail template uses spark.name, spark.namespace | Use observability chart values |
