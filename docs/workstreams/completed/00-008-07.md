...
execute_group() {
    ...
    for log_file in "$PARALLEL_LOG_DIR"/*.log; do
        ...
        if [[ "$exit_code" -eq 0 ]]; then
            ...
            echo "❌ $scenario_name (exit: $exit_code)"
        fi
    done
...
- [x] AC1: GPU scenarios skip when no GPU nodes — ✅ (parallel.sh check_gpu_nodes)
- [x] AC2: Iceberg scenarios skip when no Hive Metastore — ✅ (check_hive_metastore)
- [x] AC3: Parallel logs aggregated — ✅ (AGGREGATE_LOGS)
- [x] AC4: --retry flag for failed scenarios — ✅
- [x] AC5: Resource-aware scheduling — ✅ (detect_cluster_resources)
...
| parallel.sh | exists | ✅ | ✅ |
| run-smoke-tests.sh --parallel, --retry | ✅ | ✅ | ✅ |
| TODO/FIXME | 0 | 0 | ✅ |
