...
**Acceptance Criteria:**
- [x] AC1: docker/docker-intermediate/python-deps/Dockerfile created
- [x] AC2: requirements-base.txt for core PySpark libraries
- [x] AC3: requirements-gpu.txt for NVIDIA RAPIDS (cudf, cuml)
- [x] AC4: requirements-iceberg.txt for pyiceberg support
- [x] AC5: Build script supports variant selection
- [x] AC6: Tests validate library imports
...
### Completed Steps
1. Created directory structure: `/home/fall_out_bug/work/s7/spark_k8s/docker/docker-intermediate/python-deps/`
2. Created requirements files (base, gpu, iceberg)
3. Created Dockerfile with variant support via build args
4. Created build.sh script with variant selection
5. Created test.sh script with comprehensive test coverage
6. Created README.md with usage documentation

### Files Created
| File | Lines | Description |
|------|-------|-------------|
| `Dockerfile` | 77 | Main image definition with variant support |
| `build.sh` | 79 | Build script with variant selection |
| `test.sh` | 197 | Test script covering all variants |
| `README.md` | 89 | Documentation and usage guide |
| `requirements-base.txt` | 9 | Core PySpark packages |
| `requirements-gpu.txt` | 6 | NVIDIA RAPIDS packages |
| `requirements-iceberg.txt` | 4 | Apache Iceberg packages |

### Test Results
- **Base variant**: 7/7 tests passed (PySpark 4.1.1, pandas, numpy, pyarrow)
- **Iceberg variant**: 8/8 tests passed (+ pyiceberg 0.10.0)
- **GPU variant**: Built but requires CUDA runtime for import tests
...
- Executable scripts have proper shebang ✓

### Known Limitations
- GPU variant image is large (~4-5GB) due to RAPIDS libraries
- GPU import tests require CUDA runtime in container environment
- Base Python packages (pyspark, pandas, numpy) may already exist in custom Spark builds
...
- [x] AC1-AC6: python-deps Dockerfile, requirements, build.sh, test.sh — ✅
...
|-------|--------|--------|--------|
| Goal Achievement | 100% | 6/6 AC | ✅ |
| python-deps/ | exists | Dockerfile, requirements-*, build.sh, test.sh | ✅ |
| bash -n test.sh | pass | ✅ | ✅ |
