---
ws_id: 025-12
feature: F25
status: completed
size: LARGE
project_id: 00
github_issue: null
assignee: null
depends_on: ["025-03", "025-04"]
---

## WS-025-12: Spark Job Tracing + Profiling Dashboards and Recipes

### 🎯 Goal

**What must WORK after completing this WS:**
- OpenTelemetry tracing интегрирован в Spark Connect конфигурацию
- 2 новых Grafana дашборда: tracing timeline + profiling bottlenecks
- 5 рецептов диагностики типовых проблем производительности
- Event log profiling через History Server доступен и задокументирован

**Acceptance Criteria:**
- [ ] AC1: spark-connect-configmap.yaml поддерживает OTel tracing config (conditional)
- [ ] AC2: values.yaml содержит `monitoring.openTelemetry` конфигурацию (endpoint, protocol, sampling)
- [ ] AC3: Custom SparkListener JAR для resource-wait tracking (app_start → first_executor delta)
- [ ] AC4: Hadoop S3A метрики экспортируются в Prometheus (bytes_written, op_duration, multipart)
- [ ] AC5: Grafana дашборд "Job Phase Timeline" (Gantt: resource wait / compute / I/O breakdown)
- [ ] AC6: Grafana дашборд "Spark Profiling" (GC time, ser/deser, shuffle skew, spill, idle)
- [ ] AC7: Airflow trace_id propagation через gRPC metadata → Spark spans
- [ ] AC8: Рецепт `docs/recipes/troubleshoot/slow-spark-jobs.md` создан
- [ ] AC9: Рецепт `docs/recipes/troubleshoot/data-skew-diagnosis.md` создан
- [ ] AC10: Рецепт `docs/recipes/troubleshoot/oom-and-spill.md` создан
- [ ] AC11: Рецепт `docs/recipes/troubleshoot/shuffle-bottleneck.md` создан
- [ ] AC12: Рецепт `docs/recipes/troubleshoot/gc-pressure.md` создан
- [ ] AC13: Рецепт `docs/recipes/troubleshoot/resource-wait-long.md` (K8s resource starvation)
- [ ] AC14: Рецепт `docs/recipes/troubleshoot/s3-write-slow.md` (S3 I/O bottleneck)
- [ ] AC15: `docs/recipes/observability/spark-profiling-guide.md` создан (обзорный guide)
- [ ] AC16: helm template проходит с включённым OTel tracing
- [ ] AC17: Обновлён `docs/recipes/observability/observability-stack.md` (ссылки на новые дашборды)

**WS is NOT complete until Goal is achieved (all AC checked).**

### Reference Case: Airflow ETL 2h15m

```
Airflow Task (wall clock: 2h 15m)
│
├─ [Phase 0]  Airflow → gRPC submit                ~seconds
├─ [Phase 1]  Driver ждёт executors от K8s          ~long       ← INVISIBLE
├─ [Phase 2]  Compute (stages, tasks, shuffle)       ~short
└─ [Phase 3]  S3 write (multipart upload)            ~long      ← looks like "slow tasks"
```

**4-layer tracing model:**

| Layer | Source | What it captures |
|-------|--------|-----------------|
| Airflow | OTel spans (native 2.7+) | Task wall clock, DAG context, trace_id |
| Spark Connect | gRPC interceptor | Submit → response, trace_id propagation |
| Spark Driver | Custom SparkListener | Resource wait (app_start → first_executor), job/stage/task spans |
| S3/MinIO | Hadoop S3A metrics | Write throughput, multipart latency, op count |

**Key insight:** Resource wait (Phase 1) is completely invisible without custom
SparkListener that tracks `onApplicationStart` → `onExecutorAdded` delta.
S3 write bottleneck (Phase 3) visible when write-stage task p95 >> compute-stage task p95.

**Custom SparkListener metrics:**

| Metric | Meaning |
|--------|---------|
| `spark_resource_wait_seconds` | Time from app start to first executor |
| `spark_phase_compute_seconds` | Time in compute stages |
| `spark_phase_io_write_seconds` | Time in write stages (output > 0 bytes) |
| `spark_phase_breakdown_ratio` | wait:compute:io ratio |

**Diagnosis table:**

| Symptom | Metric | Root Cause | Fix |
|---------|--------|------------|-----|
| resource_wait > 300s | spark_resource_wait_seconds | K8s scheduling delay | Priority class, pre-warm pool, batch allocation |
| write_stage p95 >> compute p95 | task duration histogram | S3 I/O bound | Increase multipart.size, magic committer, reduce partitions |
| compute p95 high variance | task duration histogram | Data skew | Repartition, salting, AQE |

### Grafana Dashboard: Job Phase Timeline

```
┌──────────────────────────────────────────────────────────┐
│  Spark Job Phase Analysis                                  │
├──────────────────────────────────────────────────────────┤
│  [Gantt Timeline]                                          │
│  ┌─────────────────┬────┬─────────────────┐                │
│  │  Resource Wait  │Comp│   S3 Write      │                │
│  │  47% (1h 00m)   │ 6% │  47% (1h 00m)   │                │
│  └─────────────────┴────┴─────────────────┘                │
│  ↑ app_start  ↑ first_exec  ↑ write_stage  ↑ job_end      │
│                                                            │
│  [Phase Breakdown]        [Key SLIs]                       │
│  Wait:    47%             Time to 1st executor: 3600s      │
│  Compute:  6%             Compute efficiency: 6%           │
│  I/O:     47%             S3 throughput: 2.8 MB/s          │
│                           Task p95 compute: 12s            │
│  [Executor Lifecycle]     Task p95 write: 890s             │
│  0────1────5──5──0                                         │
│  pend | scale | drain     [Airflow Context]                │
│                           DAG: etl_daily task: transform   │
│                           trace_id: abc123                 │
└──────────────────────────────────────────────────────────┘
```

### Grafana Dashboard: Spark Profiling

```
┌──────────────────────────────────────────────────────┐
│  Spark Profiling                                       │
├──────────────────────────────────────────────────────┤
│                                                        │
│  [GC Time %]             [Serialization Time]          │
│  ┌──────────┐            ┌──────────┐                  │
│  │ ▓▓▓░░░░░ │ 15%        │ ▓░░░░░░░ │ 3%              │
│  └──────────┘            └──────────┘                  │
│                                                        │
│  [Shuffle Skew]          [Spill to Disk]               │
│  ┌──────────┐            ┌──────────┐                  │
│  │ max/avg  │ 4.1x       │ 2.3 GB   │                  │
│  └──────────┘            └──────────┘                  │
│                                                        │
│  [Executor Utilization]  [Peak Memory per Executor]    │
│  ┌──────────────┐        ┌──────────────┐              │
│  │ ▓▓▓▓░░░░░░░░ │ 38%    │ ▓▓▓▓▓▓▓▓░░░ │ 78%          │
│  └──────────────┘        └──────────────┘              │
│                                                        │
│  [Task Duration Heatmap]                               │
│  Executor0 ▓▓▓▓▓▓▓░░░                                 │
│  Executor1 ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓                          │
│  Executor2 ▓▓▓▓▓▓▓▓░░░░                               │
└──────────────────────────────────────────────────────┘
```

### Recipes Overview

| Recipe | Problem | Key Metrics | Action |
|--------|---------|-------------|--------|
| slow-spark-jobs | Job takes too long | task duration p95, stage time | identify bottleneck stage |
| data-skew-diagnosis | Uneven data distribution | max/median task size ratio | repartition, salting |
| oom-and-spill | Executor OOM, spill | memory used, spill bytes | tune memory, partitions |
| shuffle-bottleneck | Slow shuffle | shuffle read/write time, bytes | optimize partitioning |
| gc-pressure | High GC pause | GC time %, full GC count | tune JVM, reduce objects |

### OpenTelemetry Configuration

```yaml
# values.yaml addition
monitoring:
  openTelemetry:
    enabled: false
    endpoint: "http://otel-collector:4317"
    protocol: "grpc"           # grpc or http
    samplingRatio: "1.0"       # 0.0-1.0
    serviceName: "spark-connect-35"
    exporterType: "otlp"       # otlp or jaeger
```

Spark 3.5 config properties:
```
spark.plugins=org.apache.spark.sql.connect.SparkConnectPlugin
spark.opentelemetry.enabled=true
spark.otel.exporter.otlp.endpoint=...
spark.otel.resource.attributes=service.name=spark-connect-35
```

### Files Changed

**Chart templates:**
- `charts/spark-3.5/templates/spark-connect-configmap.yaml` (ADD OTel + S3A metrics config)
- `charts/spark-3.5/values.yaml` (ADD monitoring.openTelemetry, monitoring.s3aMetrics)
- `charts/spark-3.5/templates/monitoring/grafana-dashboard-job-phase-timeline.yaml` (NEW)
- `charts/spark-3.5/templates/monitoring/grafana-dashboard-profiling.yaml` (NEW)

**Custom SparkListener (resource wait tracker):**
- `docker/spark-3.5/listeners/resource-wait-tracker.py` (NEW) — PySpark listener
- OR `spark-listeners/src/ResourceWaitTracker.java` (NEW) — JAR plugin

**Troubleshooting recipes (7):**
- `docs/recipes/troubleshoot/slow-spark-jobs.md` (NEW)
- `docs/recipes/troubleshoot/data-skew-diagnosis.md` (NEW)
- `docs/recipes/troubleshoot/oom-and-spill.md` (NEW)
- `docs/recipes/troubleshoot/shuffle-bottleneck.md` (NEW)
- `docs/recipes/troubleshoot/gc-pressure.md` (NEW)
- `docs/recipes/troubleshoot/resource-wait-long.md` (NEW)
- `docs/recipes/troubleshoot/s3-write-slow.md` (NEW)

**Observability guides:**
- `docs/recipes/observability/spark-profiling-guide.md` (NEW)
- `docs/recipes/observability/observability-stack.md` (UPDATE)
