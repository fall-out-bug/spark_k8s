...
1. **standalone** (parent) — `standalone.enabled=true`
   - Только master + workers (templates/spark-standalone.yaml)
   - Без Airflow

2. **standalone component** — `standalone.enabled=true`
...
## Airflow — почему в standalone component
...
- Матрица использует **standalone** (parent), subchart отключён (`standalone.enabled=false`)
