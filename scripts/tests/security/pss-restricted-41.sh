#!/bin/bash
# @meta
# name: "pss-restricted-41"
# type: "security"
# description: "PSS restricted compliance for Spark 4.1"
# category: "pss"
# profile: "restricted"
# spark_version: "4.1"
...
RELEASE_NS="${RELEASE_NS:-pss-restricted-41-test}"
RELEASE_NAME="${RELEASE_NAME:-spark-41-pss-test}"
...
deploy_spark() {
    echo "Deploying Spark 4.1 with PSS restricted..."
...
validate_pss_compliance() {
    ...
    if ! validate_pss_restricted "$pod_name" "$RELEASE_NS"; then
        echo "ERROR: PSS restricted validation failed"
        return 1
    fi
}
...
main() {
    echo "=== PSS Restricted Test for Spark 4.1 ==="
