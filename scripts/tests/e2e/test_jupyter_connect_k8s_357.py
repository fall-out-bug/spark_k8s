"""
Jupyter connect-k8s E2E tests for Spark 3.5.7.

Tests validate Spark 3.5.7 execution via Jupyter with Spark Connect in k8s mode.
"""
import pytest
...
class TestJupyterConnectK8s357:
    """E2E tests for Jupyter with Spark 3.5.7 in connect-k8s mode."""
    ...
    def test_q4_window(
        ...
        metrics = query_metrics(
            """SELECT passenger_count, fare_amount,
               ROW_NUMBER() OVER (PARTITION BY passenger_count ORDER BY fare_amount DESC) AS fare_rank
            FROM nyc_taxi
            WHERE passenger_count BETWEEN 1 AND 3 AND total_amount > 0 AND fare_amount > 0
