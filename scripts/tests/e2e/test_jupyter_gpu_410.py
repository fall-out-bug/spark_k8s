"""
Jupyter GPU E2E tests for Spark 4.1.0.

Tests validate Spark 4.1.0 execution via Jupyter with GPU acceleration (RAPIDS).
Tests use Spark 4.1.0 which includes native GPU support.
"""
import pytest
...
class TestJupyterGPU410:
    """E2E tests for Jupyter with GPU acceleration on Spark 4.1.0."""

    @pytest.fixture(autouse=True)
    def setup_gpu(self, spark_session_with_gpu, sample_dataset_path):
        """Setup GPU-enabled session and load dataset."""
        self.spark = spark_session_with_gpu
        self.dataset_path = sample_dataset_path
    ...
    def test_gpu_q1_count(self, spark_session_with_gpu, sample_dataset_path, query_metrics, gpu_metrics):
        ...
        gpu_result = gpu_metrics
        assert gpu_result["gpu_utilization"] >= 0, "GPU metrics collected"
    ...
    def test_gpu_q4_sort(self, spark_session_with_gpu, sample_dataset_path, query_metrics, gpu_metrics):
        ...
        assert metrics["execution_time"] < 600, "Sort query took too long"
...
class TestJupyterGPU410FullDataset:
