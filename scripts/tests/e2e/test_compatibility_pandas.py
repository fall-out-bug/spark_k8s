...
class TestPandasCompatibility:
    ...
    def test_pandas_conversion(self, spark_session, pandas_compatibility, library_versions):
        """Test pandas to Spark DataFrame conversion."""
        result = pandas_compatibility

        assert result["conversion_success"], f"pandas conversion failed: {result.get('error', 'unknown error')}"
    ...
    def test_pandas_types(self, spark_session, type_compatibility):
        ...
        assert result["column_count"] == 4, f"Expected 4 columns, got {result['column_count']}"

    def test_pandas_version_check(self, library_versions):
    ...
    def test_pandas_dataframe_operations(self, spark_session):
        """Test PySpark operations on pandas-converted DataFrames."""
        try:
            import pandas as pd

            # Create pandas DataFrame
            pdf = pd.DataFrame({"group": ["A", "A", "B", "B", "C"], "value": [10, 20, 30, 40, 50]})
