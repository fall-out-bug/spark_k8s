"""
Security stability load test: PSS restricted mode under load.

Scenario: Validate PSS restricted policies remain stable under sustained load.
Duration: 30 minutes
"""
...
def test_pss_restricted_stability(spark_connect_client):
    """
    Security stability test: PSS restricted mode at 1 qps for 30 minutes.
    ...
    metrics = {
        "query_name": "pss_restricted_stability",
        "queries_total": 0,
        "queries_success": 0,
        "queries_failed": 0,
        "latencies": [],
    ...
    print(f"[pss_restricted] Starting sustained load: {duration_sec}s, {interval_sec}s interval")
    ...
    while datetime.now() < end_time:
        try:
            ...
            # Check pod security context every 60 seconds
                    ...
                    for pod in pods.items:
                        ...
                        for container in pod.spec.containers:
                            if container.security_context and container.security_context.privileged:
                                ...
                                print(f"WARNING: Privileged container found: {pod.metadata.name}/{container.name}")

                        # Verify runAsUser is set (not root)
                        if pod.spec.security_context:
                            if pod.spec.security_context.run_as_user == 0:
                                metrics["policy_violations"] += 1
                                print(f"WARNING: Root user container found: {pod.metadata.name}")

                except Exception as e:
                    print(f"WARNING: Could not check pod security: {e}")

        except Exception as e:
            metrics["queries_failed"] += 1
            print(f"[pss_restricted] Query failed: {e}")
        finally:
    ...
    assert metrics["policy_violations"] == 0, \
        f"Policy violations detected: {metrics['policy_violations']}"
