...
# Usage: NAMESPACE=spark-matrix-scenario-0009 RELEASE=scenario0009 DEPLOY_MODE=connect ./run-smoke-against-release.sh
...
case "$DEPLOY_MODE" in
    ...
    k8s-native)
        ...
        kubectl cp "$smoke_script" "$NAMESPACE/$submitter_pod:/tmp/smoke_1k.py"
