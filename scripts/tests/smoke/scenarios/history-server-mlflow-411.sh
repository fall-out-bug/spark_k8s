#!/bin/bash
# Smoke test: History Server validation for MLflow (Spark 4.1.1)

# @meta
# name: "history-server-mlflow-411"
# type: "smoke"
# description: "History Server validation for MLflow scenarios (Spark 4.1.1)"
# version: "4.1.1"
...
# depends_on: []
# tags: [smoke, history-server, mlflow, 4.1.1]
# @endmeta
...
APP_NAME="mlflow-history-test-411"
...
setup_test_environment() {
    ...
    local env_setup
    env_setup=$(setup_test_environment "hs-mlflow" "411")
...
main() {
    log_section "Smoke Test: History Server + MLflow (4.1.1)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
