#!/bin/bash
# Smoke test: Jupyter + Spark Connect + K8s backend + GPU + Iceberg (Spark 4.1.1)

# @meta
# name: "jupyter-connect-k8s-gpu-iceberg-411"
# type: "smoke"
# description: "Smoke test for Jupyter + Spark Connect + K8s backend + GPU + Iceberg (Spark 4.1.1)"
# version: "4.1.1"
# component: "jupyter"
# mode: "connect-k8s"
...
# depends_on: []
# tags: [smoke, jupyter, connect-k8s, gpu, iceberg, 4.1.1]
# @endmeta
...
setup_test_environment() {
    ...
    local env_setup
    env_setup=$(setup_test_environment "jupyter-connect-k8s-gpu-iceberg" "411")
...
deploy_spark() {
    log_section "Deploying Jupyter with Spark Connect + GPU + Iceberg support"
...
validate_deployment() {
    ...
    log_success "All components validated with GPU + Iceberg"
}
...
run_smoke_test() {
    ...
    if check_iceberg_available "$TEST_NAMESPACE"; then
        iceberg_conf="--conf spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions --conf spark.sql.catalog.spark_catalog=org.apache.iceberg.spark.SparkSessionCatalog"
    fi
...
main() {
    log_section "Smoke Test: Jupyter + Spark Connect + GPU + Iceberg (4.1.1)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
