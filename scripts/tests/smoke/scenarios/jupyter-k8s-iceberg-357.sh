#!/bin/bash
# Smoke test: Jupyter + K8s submit mode + Iceberg (Spark 3.5.7)

# @meta
# name: "jupyter-k8s-iceberg-357"
# type: "smoke"
# description: "Smoke test for Jupyter + K8s submit mode + Iceberg (Spark 3.5.7)"
# version: "3.5.7"
# component: "jupyter"
# mode: "k8s-submit"
# features: ["iceberg"]
...
# depends_on: []
# tags: [smoke, jupyter, k8s-submit, iceberg, 3.5.7]
# @endmeta
...
setup_test_environment() {
    ...
    local env_setup
    env_setup=$(setup_test_environment "jupyter-k8s-iceberg" "357")
...
main() {
    log_section "Smoke Test: Jupyter + K8s Submit + Iceberg (3.5.7)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
