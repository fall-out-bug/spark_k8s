#!/bin/bash
# Smoke test: Airflow + Spark Connect + Standalone backend (Spark 3.5.7)

# @meta
# name: "airflow-connect-standalone-357"
# type: "smoke"
# description: "Smoke test for Airflow + Spark Connect + Standalone backend (Spark 3.5.7)"
# version: "3.5.7"
# component: "airflow"
# mode: "connect-standalone"
# features: []
# chart: "charts/spark-3.5"
# preset: "charts/spark-3.5/airflow-connect-standalone-3.5.7.yaml"
# estimated_time: "5 min"
# depends_on: []
# tags: [smoke, airflow, connect-standalone, 3.5.7]
# @endmeta
...
PRESET_PATH="${PROJECT_ROOT}/charts/spark-3.5/airflow-connect-standalone-3.5.7.yaml"
SPARK_VERSION="3.5.7"
IMAGE_TAG="3.5.7"
...
setup_test_environment() {
    ...
    local env_setup
    env_setup=$(setup_test_environment "airflow-connect-sa" "357")
...
deploy_spark() {
    log_section "Deploying Spark Connect + Airflow (Standalone mode)"
...
print_results() {
    ...
    echo "Configuration: Spark $SPARK_VERSION, Airflow + Connect + Standalone"
    kubectl get pods -n "$TEST_NAMESPACE" -o wide
}

main() {
    log_section "Smoke Test: Airflow + Spark Connect + Standalone (3.5.7)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
