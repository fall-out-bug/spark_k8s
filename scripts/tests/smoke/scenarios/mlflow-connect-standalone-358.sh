#!/bin/bash
# Smoke test: MLflow + Spark Connect + Standalone backend (Spark 3.5.8)

# @meta
# name: "mlflow-connect-standalone-358"
# type: "smoke"
# description: "Smoke test for MLflow + Spark Connect + Standalone backend (Spark 3.5.8)"
# version: "3.5.8"
# component: "mlflow"
# mode: "connect-standalone"
# features: []
# chart: "charts/spark-3.5"
# preset: "charts/spark-3.5/mlflow-connect-standalone-3.5.8.yaml"
# estimated_time: "5 min"
# depends_on: []
# tags: [smoke, mlflow, connect-standalone, 3.5.8]
# @endmeta
...
PRESET_PATH="${PROJECT_ROOT}/charts/spark-3.5/mlflow-connect-standalone-3.5.8.yaml"
SPARK_VERSION="3.5.8"
IMAGE_TAG="3.5.8"
...
setup_test_environment() {
    ...
    # Create release name
    local release_name
    release_name="$(create_release_name "mlflow-connect-standalone-" "$(generate_short_test_id)")"
...
deploy_spark() {
    log_section "Deploying Spark Connect + MLflow (Standalone mode)"
...
print_results() {
    ...
    echo "Configuration: Spark $SPARK_VERSION, MLflow + Connect + Standalone"
    kubectl get pods -n "$TEST_NAMESPACE" -o wide
}

main() {
    log_section "Smoke Test: MLflow + Spark Connect + Standalone (3.5.8)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
