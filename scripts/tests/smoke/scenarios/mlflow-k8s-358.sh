#!/bin/bash
# Smoke test: MLflow + K8s backend (Spark 3.5.8)

# @meta
# name: "mlflow-k8s-358"
# type: "smoke"
# description: "Smoke test for MLflow + K8s submit mode (Spark 3.5.8)"
# version: "3.5.8"
# component: "mlflow"
# mode: "k8s-submit"
# features: []
...
# depends_on: []
# tags: [smoke, mlflow, k8s-submit, 3.5.8]
# @endmeta
...
setup_test_environment() {
    ...
    # Create release name
    local release_name
    release_name="$(create_release_name "mlflow-k8s-" "$(generate_short_test_id)")"
...
deploy_spark() {
    log_section "Deploying MLflow (K8s submit mode)"
...
print_results() {
    ...
    echo "Configuration: Spark $SPARK_VERSION, MLflow + K8s submit"
    kubectl get pods -n "$TEST_NAMESPACE" -o wide
}

main() {
    log_section "Smoke Test: MLflow + K8s submit (3.5.8)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
