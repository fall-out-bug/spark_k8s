#!/bin/bash
# Smoke test: MLflow + Iceberg + GPU (Spark 3.5.7)

# @meta
# name: "mlflow-iceberg-gpu-357"
# type: "smoke"
# description: "Smoke test for MLflow with Iceberg and GPU support (Spark 3.5.7)"
# version: "3.5.7"
# component: "mlflow"
# mode: "iceberg-gpu"
...
# depends_on: []
# tags: [smoke, mlflow, iceberg, gpu, 3.5.7]
# @endmeta
...
APP_NAME="mlflow-iceberg-gpu-test-357"
...
setup_test_environment() {
    ...
    # Create release name
    local release_name
    release_name="$(create_release_name "mlflow-iceberg-gpu-" "$(generate_short_test_id)")"
...
deploy_spark() {
    log_section "Deploying MLflow with Iceberg and GPU support"
...
validate_deployment() {
    ...
    log_success "MLflow pod ready with GPU and Hive Metastore: $mlflow_pod"
}
...
print_results() {
    ...
    echo "Configuration: Spark $SPARK_VERSION, MLflow + Iceberg + GPU"
    kubectl get pods -n "$TEST_NAMESPACE" -o wide
}

main() {
    log_section "Smoke Test: MLflow + Iceberg + GPU (3.5.7)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
