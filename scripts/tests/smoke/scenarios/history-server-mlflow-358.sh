#!/bin/bash
# Smoke test: History Server validation for MLflow (Spark 3.5.8)

# @meta
# name: "history-server-mlflow-358"
# type: "smoke"
# description: "History Server validation for MLflow scenarios (Spark 3.5.8)"
# version: "3.5.8"
...
# depends_on: []
# tags: [smoke, history-server, mlflow, 3.5.8]
# @endmeta
...
APP_NAME="mlflow-history-test-358"
...
setup_test_environment() {
    ...
    local env_setup
    env_setup=$(setup_test_environment "hs-mlflow" "358")
...
main() {
    log_section "Smoke Test: History Server + MLflow (3.5.8)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
