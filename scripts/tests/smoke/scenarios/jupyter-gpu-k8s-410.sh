#!/bin/bash
# Smoke test: Jupyter + GPU (Spark 4.1.0)

# @meta
# name: "jupyter-gpu-k8s-410"
# type: "smoke"
# description: "Smoke test for Jupyter with GPU support (Spark 4.1.0)"
# version: "4.1.0"
# component: "jupyter"
# mode: "gpu"
# features: ["gpu"]
...
setup_test_environment() {
    ...
    # Create release name
    local release_name
    release_name="$(create_release_name "jupyter-gpu-k8s-" "$(generate_short_test_id)")"
...
deploy_spark() {
    log_section "Deploying Jupyter with GPU support"
...
print_results() {
    ...
    echo "Configuration: Spark $SPARK_VERSION, Jupyter + GPU"
    kubectl get pods -n "$TEST_NAMESPACE" -o wide
}

main() {
    log_section "Smoke Test: Jupyter + GPU (4.1.0)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
