#!/bin/bash
# Smoke test: History Server validation for MLflow (Spark 4.1.0)

# @meta
# name: "history-server-mlflow-410"
# type: "smoke"
# description: "History Server validation for MLflow scenarios (Spark 4.1.0)"
# version: "4.1.0"
...
# depends_on: []
# tags: [smoke, history-server, mlflow, 4.1.0]
# @endmeta
...
APP_NAME="mlflow-history-test-410"
...
setup_test_environment() {
    ...
    local env_setup
    env_setup=$(setup_test_environment "hs-mlflow" "410")
...
main() {
    log_section "Smoke Test: History Server + MLflow (4.1.0)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
