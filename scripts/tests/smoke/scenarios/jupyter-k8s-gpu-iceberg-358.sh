#!/bin/bash
# Smoke test: Jupyter + K8s submit mode + GPU + Iceberg (Spark 3.5.8)

# @meta
# name: "jupyter-k8s-gpu-iceberg-358"
# type: "smoke"
# description: "Smoke test for Jupyter + K8s submit mode + GPU + Iceberg (Spark 3.5.8)"
# version: "3.5.8"
# component: "jupyter"
# mode: "k8s-submit"
...
# depends_on: []
# tags: [smoke, jupyter, k8s-submit, gpu, iceberg, 3.5.8]
# @endmeta
...
setup_test_environment() {
    ...
    local env_setup
    env_setup=$(setup_test_environment "jupyter-k8s-gpu-iceberg" "358")
...
deploy_spark() {
    log_section "Deploying Jupyter with GPU + Iceberg support (K8s submit mode)"
...
validate_deployment() {
    ...
    log_success "Jupyter pod ready with GPU + Iceberg: $jupyter_pod"
}
...
main() {
    log_section "Smoke Test: Jupyter + K8s Submit + GPU + Iceberg (3.5.8)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
