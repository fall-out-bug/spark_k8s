...
PRESET_PATH="${PROJECT_ROOT}/charts/spark-4.1/presets/test-baseline-411-values.yaml"
SPARK_VERSION="4.1.1"
IMAGE_TAG="4.1.1"
...
setup_test_environment() {
    ...
    local env_setup
    env_setup=$(setup_test_environment "jupyter-connect-k8s" "411")
...
main() {
    log_section "Smoke Test: Jupyter + Spark Connect + K8s (4.1.1)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
