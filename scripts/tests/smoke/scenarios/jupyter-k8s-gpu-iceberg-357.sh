#!/bin/bash
# Smoke test: Jupyter + K8s submit mode + GPU + Iceberg (Spark 3.5.7)

# @meta
# name: "jupyter-k8s-gpu-iceberg-357"
# type: "smoke"
# description: "Smoke test for Jupyter + K8s submit mode + GPU + Iceberg (Spark 3.5.7)"
# version: "3.5.7"
# component: "jupyter"
# mode: "k8s-submit"
...
# depends_on: []
# tags: [smoke, jupyter, k8s-submit, gpu, iceberg, 3.5.7]
# @endmeta
...
setup_test_environment() {
    ...
    local env_setup
    env_setup=$(setup_test_environment "jupyter-k8s-gpu-iceberg" "357")
...
main() {
    log_section "Smoke Test: Jupyter + K8s Submit + GPU + Iceberg (3.5.7)"
    setup_test_environment
    deploy_spark
    validate_deployment
    run_smoke_test
