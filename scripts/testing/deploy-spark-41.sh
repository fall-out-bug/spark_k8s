...
kubectl create configmap hive-init-41 -n "$NAMESPACE" \
  --from-literal=hive-site.xml='<?xml version="1.0" encoding="UTF-8"?>
<configuration>
  <property>
    <name>javax.jdo.option.ConnectionURL</name>
    <value>jdbc:postgresql://shared-spark-base-postgresql:5432/metastore_spark41</value>
  </property>
  <property>
...
# Run schematool
kubectl run "hive-init-41-$$" \
    --restart=Never \
    --image=spark-k8s/hive:4.0.0-pg \
    -n "$NAMESPACE" \
    --env="HIVE_CONF_DIR=/tmp/hive-conf" \
    --overrides='{"spec":{"volumes":[{"name":"hive-config","configMap":{"name":"hive-init-41"}}],"containers":[{"name":"hive-init-41-$$","volumeMounts":[{"name":"hive-config","mountPath":"/etc/hive-config"}]}]}}' \
    --command -- /bin/bash -lc "
...
# Wait for completion
for i in {1..60}; do
    if kubectl get pod "hive-init-41-$$" -n "$NAMESPACE" &>/dev/null; then
        PHASE=$(kubectl get pod "hive-init-41-$$" -n "$NAMESPACE" -o jsonpath='{.status.phase}')
        if [ "$PHASE" = "Succeeded" ]; then
            break
        fi
        if [ "$PHASE" = "Failed" ]; then
            echo -e "${RED}✗ Schema initialization failed${NC}"
            kubectl logs "hive-init-41-$$" -n "$NAMESPACE" | tail -20
            kubectl delete pod "hive-init-41-$$" -n "$NAMESPACE" --ignore-not-found=true &>/dev/null
            exit 1
        fi
    fi
    sleep 2
...
kubectl delete pod "hive-init-41-$$" -n "$NAMESPACE" --ignore-not-found=true &>/dev/null
kubectl delete configmap hive-init-41 -n "$NAMESPACE" --ignore-not-found=true &>/dev/null
