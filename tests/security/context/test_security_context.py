...
class TestSecurityContext:
    ...
    def test_pss_restricted_in_prod(self, repository_root):
        ...
        assert values["security"]["podSecurityStandards"] == True, "PSS restricted should be enabled in prod"
    ...
    def test_non_root_user(self, repository_root):
        ...
        assert values["security"]["runAsUser"] == 185, "Should run as non-root user (UID 185)"
        assert values["security"]["runAsGroup"] == 185, "Should run as non-root group (GID 185)"
    ...
    def test_readonly_root_filesystem_option(self, repository_root):
        ...
        assert "readOnlyRootFilesystem" in values["security"], "ReadOnlyRootFilesystem should be defined"
    ...
    def test_privilege_escalation_disabled(self, repository_root):
        ...
        assert (
            "allowPrivilegeEscalation: false" in content or "privileged: false" in content
