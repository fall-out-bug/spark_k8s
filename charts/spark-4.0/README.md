# Apache Spark 4.0 on Kubernetes

[![Spark Version](https://img.shields.io/badge/Spark-4.0.2-orange)](https://spark.apache.org/)
[![Helm](https://img.shields.io/badge/Helm-3.x-blue)](https://helm.sh)

Unified Helm chart for deploying Apache Spark 4.0.0 on Kubernetes with support for Spark Connect, GPU acceleration (RAPIDS), Apache Iceberg, Jupyter, and Airflow.
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/presets/core-baseline.yaml
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/presets/core-gpu.yaml
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/presets/core-iceberg.yaml
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/presets/core-gpu-iceberg.yaml
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/values-scenario-jupyter-connect-k8s.yaml
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/values-scenario-jupyter-connect-standalone.yaml
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/values-scenario-airflow-connect-k8s.yaml
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/values-scenario-airflow-connect-standalone.yaml
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/values-scenario-airflow-k8s-submit.yaml
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/values-scenario-airflow-operator.yaml
```
...
3. **REST Catalog** (new in Spark 4.0):
```yaml
features:
  iceberg:
    enabled: true
...
Migrating from Spark 3.5 to Spark 4.0 requires several changes due to API updates and new features.
...
# Spark 4.0 (additional shuffle options)
features:
  gpu:
    rapids:
      ...
      sql:
        enabled: true
      shuffle:
        enabled: true  # New in 4.0
```

#### 7. Update Iceberg REST Catalog (optional, new in 4.0)
...
# Spark 4.0 (can use REST catalog)
features:
  iceberg:
    catalogType: "rest"
    uri: "http://iceberg-rest-catalog:8181"
...
```bash
# Install Spark 4.0 in test namespace
helm install spark-test charts/spark-4.0 \
  -f charts/spark-4.0/presets/core-baseline.yaml \
  --namespace test
...
```bash
helm upgrade my-spark charts/spark-4.0 \
  -f charts/spark-4.0/presets/core-baseline.yaml
```
