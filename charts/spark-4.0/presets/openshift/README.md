# OpenShift Presets for Spark 4.0

This directory contains OpenShift-specific configuration presets for deploying Spark 4.0 on OpenShift clusters.
...
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/presets/openshift/restricted.yaml \
  -n spark-openshift \
  --create-namespace
```
...
```bash
helm install my-spark charts/spark-4.0 \
  -f charts/spark-4.0/presets/openshift/anyuid.yaml \
  -n spark-openshift \
  --create-namespace
```
...
helm install my-spark charts/spark-4.0 \
  ...
  --set hiveMetastore.postgresql.database='metastore_spark41' \
  -n spark-openshift
```
