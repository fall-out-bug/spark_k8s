...
def push_metric(name, value, labels=None):
    ...
    try:
        requests.post(f"{PUSHGATEWAY_URL}/metrics/job/spark_feature_engineering", data=metric_data, timeout=5)
...
def create_spark_session():
    ...
    spark = (
        SparkSession.builder.appName("nyc-taxi-feature-engineering")
...
def add_temporal_features(df):
    ...
    df = (
        ...
        .withColumn(
            "is_rush_hour",
            F.when((F.col("hour_of_day").between(7, 9)) | (F.col("hour_of_day").between(17, 19)), 1).otherwise(0),
        )
        .withColumn("is_holiday", F.when(F.col("pickup_date").isin(holidays_list), 1).otherwise(0))
        .withColumn(
            "time_of_day",
...
def add_geospatial_features(df):
    ...
    df = df.withColumn(
        "trip_duration", (F.unix_timestamp("tpep_dropoff_datetime") - F.unix_timestamp("tpep_pickup_datetime")) / 60
    )

    # Average speed (mph)
    df = df.withColumn(
        "avg_speed",
        F.when(F.col("trip_duration") > 0, F.col("trip_distance") / (F.col("trip_duration") / 60)).otherwise(0),
    )

    # Airport pickups (JFK=132, LGA=138, EWR=1)
    df = df.withColumn("is_airport", F.when(F.col("PULocationID").isin([1, 132, 138]), 1).otherwise(0))
...
def add_historical_features(df):
    ...
    window_7d = Window.partitionBy("PULocationID").orderBy("pickup_date").rowsBetween(-7, -1)

    daily_zone = (
        daily_zone.withColumn("revenue_ma_7d", F.avg("daily_revenue").over(window_7d))
        ...
        .withColumn(
            "trips_same_day_lw", F.lag("daily_trips", 7).over(Window.partitionBy("PULocationID").orderBy("pickup_date"))
        )
    )

    # Join back to original
    df = df.join(daily_zone, on=["pickup_date", "PULocationID", "pickup_borough"], how="left")
...
def create_target_variables(df):
    ...
    daily_zone = df.groupBy("pickup_date", "PULocationID", "pickup_borough").agg(
        F.sum("total_amount").alias("daily_revenue"), F.count("*").alias("daily_trips")
    )

    # Create 7-day forward sums
    window_7d_forward = Window.partitionBy("PULocationID").orderBy("pickup_date").rowsBetween(1, 7)

    daily_zone = daily_zone.withColumn("total_amount_7d", F.sum("daily_revenue").over(window_7d_forward)).withColumn(
        "trip_count_7d", F.sum("daily_trips").over(window_7d_forward)
    )
